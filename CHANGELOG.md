...
- 歌单歌曲点序号改为操作面板（`▶️ 播放 / 🗑 删除`），不再直接发文件：点删除 → 两步确认 → 从音乐库删除该歌（连文件删除、不可恢复），歌单自动移除该歌并刷新音乐库
- 菜单交互补返回/取消：音质选择面板新增 `🔙 返回选歌 / ❌ 取消`（选错歌不用重新 /music）；收藏播放后面板新增 `🔙 返回收藏列表`，可继续浏览收藏
- **信息卡先行**：搜索选歌下载后不再自动发送音频文件（机器人以管理音乐为主），改为返回歌曲信息卡（专辑封面 + 歌名/歌手/专辑/格式）+ 操作面板 `[▶️ 试听][📥 入库][❤️ 收藏][🗑 删除]`，点 ▶️ 试听才把文件发回；音乐库已有该歌（Emby/本地命中）同样返回信息卡 + `▶️ 试听`，不再直接发文件；`🗑 删除` 只针对临时区文件（音乐库文件删除请走 `/emby` 面板，会连同文件删除）
- **音乐库唯一入口**：`/music <歌名>` 先搜 Emby 音乐库——命中显示库内候选列表（序号横排 + 格式），点序号进入管理态信息卡；未命中显示 `🔍 在线找歌 / ❌ 取消`，点在线找歌走 sqmusic 下载流程；`/emby` 退役为兼容别名（无参数提示并入 `/music`，带关键词等同库内搜索）
- **信息卡两态统一**：音乐库歌曲同样支持 `❤️ 收藏 / ➕ 加歌单`（不再只有试听/删除）；信息卡 `🔙 返回` 按状态返回对应候选列表（管理态回库内候选、下载态回在线候选）
- **已入库歌曲两步删除**：`/music` 音乐库命中信息卡 `🗑 删除` → `✅ 确认删除 / 🔙 取消` → `DELETE /Items/{id}`（**连同音乐文件一起删除、不可恢复**）+ 自动刷新音乐库
- **Dockerfile 分层缓存**：构建改为「依赖清单层（Cargo.toml/Cargo.lock + dummy 源码预编译依赖）+ 真源码层增量编译」，改代码不再全量重编依赖（NAS 实测构建从 ~5-7 分钟降至 ~3 分钟）
- **容器化开发**：新增 `tgsync-dev` 开发镜像 + `tgsync-target`/`tgsync-registry` 持久卷，`cargo check` 在容器内执行（增量 ~0.3s），NAS 宿主不再直接编译 Rust；`.dockerignore` 排除 target/.git/server/k8s
