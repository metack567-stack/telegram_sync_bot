...
- **音乐库管理器（唯一入口）**：`/music <歌名>` **先搜 Emby 音乐库**——命中直接管理（试听/收藏/删除/加歌单），未命中一键 **🔍 在线找歌**（对接 sqmusic「简单音乐」）下载入库。**信息卡先行**：不自动发文件，返回歌曲信息卡（专辑封面 + 歌名/歌手/专辑/格式）+ 操作面板，点 **▶️试听** 才发送；支持 **📥入库**（移入音乐库 + 自动刷新 Emby）/ **❤️收藏** / **🗑删除**（两步确认，连文件删，不可恢复）/ **➕加歌单**
- **Emby 音乐库联动**：`/music` 库内优先（命中直接管理，未命中在线找歌）；下载完成自动触发 Emby 库刷新；已入库歌曲信息卡 `🗑 删除` 两步确认（**连同音乐文件一起删除，不可恢复**）；`/emby` 已并入 `/music`（兼容保留）
- **Emby 歌单**：`/playlist <歌单名>` 创建/打开 Emby 播放列表；歌曲信息卡 `➕ 加歌单` 弹出歌单列表当场选择；`/playlist` 列出全部歌单（序号按钮点选）、可查看歌单内歌曲，点序号弹出 `▶️ 播放 / 🗑 删除` 操作面板（删除=从音乐库删除该歌，**连文件一起删、不可恢复**），列表底部 `🗑 删除歌单` 两步确认即可删除
- **音乐收藏**：`/favs` 查看收藏列表，点序号播放、播放后可返回列表继续浏览或取消收藏（SQLite 持久化，跨重启保留）
...
> `/music` 是**唯一音乐入口**（音乐库管理器）：`/music <歌名>` **先搜 Emby 音乐库**（配置 `EMBY_URL`/`EMBY_API_KEY` 时）——**命中**显示库内候选列表（序号横排 + 格式），点序号进入**管理态信息卡** `[▶️ 试听][❤️ 收藏][🗑 删除][➕ 歌单][🔙 返回]`（试听才发文件，删除两步确认并连同文件删除）；**未命中**显示 `🔍 在线找歌 / ❌ 取消`，点在线找歌走 sqmusic 搜索（默认酷我源，可加音质/来源前缀 `/music flac 晴天`）→ 序号选歌 → 音质按钮 → 下载落临时区（设置 `MUSIC_TMP_DIR` 时；未设置则直接入库并刷新 Emby）→ **下载态信息卡** `[▶️ 试听][📥 入库][❤️ 收藏][🗑 删除][➕ 歌单][🔙 返回]`。**不自动发送音频文件**，点 ▶️ 试听才发回；入库 = 移入音乐库 + 自动刷新 Emby；临时文件超时由后台任务定时清理（`MUSIC_TMP_RETENTION_SECS`/`MUSIC_TMP_KEEP`）。**信息卡两态统一**：库内歌同样支持 收藏/加歌单/两步删除。`/emby` 已并入 `/music`（兼容保留：`/emby <歌名>` 等同库内搜索，无参数提示用 /music）。`/playlist <歌单名>` 创建/打开 Emby 歌单，`/playlist`（无参数）列出全部歌单点序号打开、底部可删除歌单，歌单内歌曲点序号弹出 `▶️ 播放 / 🗑 删除` 面板；`/favs` 查看收藏列表，点序号播放、可返回列表继续浏览或取消收藏。
...
**容器化开发（推荐，NAS 等宿主零污染）**：`bot/Containerfile` 的 builder 阶段可直接作为开发镜像（`docker build --target bot_builder -t tgsync-dev:latest -f bot/Containerfile .`）；编译产物与依赖缓存分别落在 `tgsync-target` / `tgsync-registry` 两个持久卷，`cargo check` 在容器内执行（增量 ~0.3s），宿主只需 docker：

```sh
docker run --rm \
  -v <repo>:/src \
  -v tgsync-target:/src/target \
  -v tgsync-registry:/usr/local/cargo/registry \
  -w /src tgsync-dev:latest cargo check
```

**构建提速**：`Containerfile` 采用分层缓存——先 COPY 依赖清单（Cargo.toml/Cargo.lock）+ dummy 源码单独预编译依赖（该层缓存），再 COPY 真源码增量编译本体；改代码不再全量重编依赖（NAS 实测构建从 ~5-7 分钟降至 ~3 分钟，其中大头是 fat LTO 链接）。`.dockerignore` 排除了 `target/.git/server/k8s`，避免把旧编译产物带进镜像。
